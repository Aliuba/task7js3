  ...
  for (const iterator of users) {
      ...
      for (const key in iterator) {
          if (iterator.hasOwnProperty(key)) {
              if (key === "address") {
                  for (const keyAddress in iterator[key]) {
                      if (iterator[key].hasOwnProperty(keyAddress)) {
                          let divProperty = document.createElement("div")
                          divMain.appendChild(divProperty)
                          let divAddress = document.createElement("div")
                          divAddress.style = "border:1px solid tomato"
                          divProperty.appendChild(divAddress)
                          divAddress.innerText = `${keyAddress}:${iterator[key][keyAddress]}`
                      }
                  }
              } else {
                  let divProperty = document.createElement("div")
                  divMain.appendChild(divProperty)
                  divProperty.innerText = `${key}:${iterator[key]}`
                  console.log(key)
                  divProperty.style = "border:1px solid tomato; background-color: green"
